...
def main(param ) :
    ...
    def find_ram_in_title(title):
        ...
        return None
    df_sa=df
    df_sa.columns = [col.lower() for col in df_sa.columns]
    df_sa.rename(columns={'main camera': 'camera', 'network': 'cellular technology','number of sim':'sim count',}, inplace=True)
    df_sa['CPU'] = df_sa.apply(lambda row: find_cpu_in_title(row['title']) or row['cpu'], axis=1)
    df_sa['color'] = df_sa.apply(lambda row: find_color_in_title(row['title']) or row['color'], axis=1)
    df_sa['Storage'] = df_sa.apply(lambda row: find_strg_in_title(row['title']) or row['storage'], axis=1)
    df_sa['RAM'] = df_sa.apply(lambda row: find_ram_in_title(row['title']) or row['ram'], axis=1)
    dfm = list(df_sa['model'].unique())
    df_sa['Model'] = df_sa.apply(lambda row: find_model_in_title(row['title'], dfm) or row['model'], axis=1)
    df_sa['ram'] = df_sa['ram'].fillna(df_sa['RAM'])
    df_sa['storage'] = df_sa['storage'].fillna(df_sa['Storage'])
    df_sa['model']= df_sa['model'].fillna(df_sa['Model'])
    df_sa['cpu'] = df_sa['cpu'].fillna(df_sa['CPU'])
    df_sa=df_sa.drop(['Storage','CPU','RAM','Model'], axis=1)
    df_sa['storage'] = df_sa['storage'].apply(convert_to_gb)
    df_sa['ram'] = df_sa['ram'].apply(convert_to_gb)
    df_sa['screen size'] = df_sa['screen size'].apply(convert_screen_size)
    df_sa[['width resolution', 'height resolution']] = df_sa['resolution'].apply(convert_resolution).apply(pd.Series)
    df_sa['price'] = df_sa['price'].apply(convert_price_before).apply(lambda x: change_price_to_USD(x, SAR_USD))
    df_sa['price before promotion'] = df_sa['price before promotion'].apply(convert_price_before).apply(lambda x: change_price_to_USD(x, SAR_USD))
    df_sa['rate'] = df_sa['rate'].apply(convert_rate)
    df_sa['currancy'] = df_sa['currancy'].apply(change_to_USD)
     
    df2 =pd.DataFrame(df_sa)  
    df2=df2.to_json(orient='columns')     
    return df2
    #return df_sa.to_dict(orient='list')
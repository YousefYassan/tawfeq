...
def main(param ) :
    ...
    dfm = list(df_eg['model'].unique())
    df_eg['Model'] = df_eg.apply(lambda row: find_model_in_title(row['title'], dfm) or row['model'], axis=1)
    df_eg['ram'] = df_eg['ram'].fillna(df_eg['RAM'])
    df_eg['storage'] = df_eg['storage'].fillna(df_eg['Storage'])
    df_eg['model']= df_eg['model'].fillna(df_eg['Model'])
    df_eg['cpu'] = df_eg['cpu'].fillna(df_eg['CPU'])
    df_eg=df_eg.drop(['Storage','CPU','RAM','Model'], axis=1)
    ...
    df2 =pd.DataFrame(df_eg)  
    df2=df2.to_json(orient='columns')     
    return df2
    #return df_eg.to_dict(orient='list')

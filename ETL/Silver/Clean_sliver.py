...
reso['resolution'] = reso['resolution'].astype(str)  # if necessary
#fact = fact.astype({'cellular technology_id': 'str', 'wireless technology_id': 'str','wireless provider_id':'str','brand_id':'str','os_id':'str','model_id':'str','cpu_id':'str','color_id':'str','storage_id':'str','screen size_id':'str','resolution_id2f':'str'})

reso.columns = reso.columns.str.replace(' ', '_')
tecsp.columns = tecsp.columns.str.replace(' ', '_')
webs.columns = webs.columns.str.replace(' ', '_')
product.columns = product.columns.str.replace(' ', '_')
fact.columns = fact.columns.str.replace(' ', '_')

#Updating Dimention  
import os
f2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Fact_Table.parquet')
if (f2['product_id'].count()>0):

    # File exists, read the existing data and concatenate with the new data
    
    f2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Fact_Table.parquet')
    p2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Product_Dimsion.parquet')
    r2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Resolution_Dimsion.parquet')
    w2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Website_Dimsion.parquet')
    t2=pd.read_parquet('abfss://####@#######.dfs.core.windows.net/Tech_special_Dimion.parquet')
    # Assuming 'df_new' is your newly scraped data
    fact = pd.concat([fact, f2], ignore_index=True)
    reso = pd.concat([reso, r2], ignore_index=True)
    webs = pd.concat([webs, w2], ignore_index=True)
    product = pd.concat([product, p2], ignore_index=True)
    tecsp = pd.concat([tecsp, t2], ignore_index=True)
    print('tttt')
